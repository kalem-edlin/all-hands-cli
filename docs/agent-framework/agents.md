...
**Curator as orchestration owner**: The curator [ref:.claude/agents/curator.md::cf5a964] owns all Claude Code infrastructure: agents, skills, hooks, commands, envoy. Any modification to orchestration components routes through the curator, which has skills loaded for each subsystem. This prevents ad-hoc changes that might break the framework's assumptions. The curator also audits changes for context efficiency, ensuring new content doesn't bloat the system.
