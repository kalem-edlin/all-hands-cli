...
import type { HookContract, TestFixture } from "../harness/index.js";
import {
...
const VALID_PROMPT_IN_PROGRESS = PROMPT_TEMPLATE(
  "in_progress",
  "Active Task",
  2,
);
...
describe("Validation Hook Contracts E2E", () => {
  ...
  beforeAll(() => {
    fixture = createFixture({
      ...
      files: {
        ...
        ".planning/test/prompts/01.md": PROMPT_TEMPLATE(
          "pending",
          "Seeded Task",
          1,
        ),
        ".planning/test/prompts/02.md": PROMPT_TEMPLATE(
          "in_progress",
          "Active Seeded",
          2,
        ),
  ...
  describe("schema hook (PostToolUse)", () => {
    it("allows after valid prompt file exists", async () => {
      // Seed valid file
      fixture.writeFile(
        ".planning/post/prompts/valid.md",
    ...
    it("allows after valid in_progress prompt file exists", async () => {
      fixture.writeFile(
        ".planning/post/prompts/active.md",
        VALID_PROMPT_IN_PROGRESS,
      );
    ...
    it("blocks after invalid prompt file exists (bad enum)", async () => {
      fixture.writeFile(
        ".planning/post/prompts/bad-status.md",
        INVALID_PROMPT_BAD_STATUS,
      );
    ...
    it("blocks after prompt file missing frontmatter", async () => {
      fixture.writeFile(
        ".planning/post/prompts/no-fm.md",
        INVALID_PROMPT_NO_FRONTMATTER,
      );
  ...
  describe("validation hook contract compliance", () => {
    ...
    beforeAll(() => {
      contractFixture = createFixture({
        ...
        files: {
          ".planning/c/prompts/01.md": PROMPT_TEMPLATE(
            "pending",
            "Contract Task",
            1,
          ),
