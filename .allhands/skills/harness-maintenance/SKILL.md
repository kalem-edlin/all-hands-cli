...
- For structural changes to `.allhands/` content files (flows, schemas, skills), **harness-maintenance** is primary
...
Per **Context is Precious**, hooks bridge Claude Code and harness.
...
| `ah tools`     | MCP integration | **Context is Precious** (lazy load MCP servers) |
...
Agent-facing YAML schemas and their TypeScript interfaces must stay in sync:
